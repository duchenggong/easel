...
- **禁止 ask_user 等阻塞式提问（硬规则）**：本 Agent 主要经网页对话驱动，ask_user / question 类工具的提问无法展示给用户，一旦调用即永久卡死。关键信息缺失时选择最合理的默认方案继续执行（如视频画幅未明确则默认 16:9 横屏、时长未定则默认 5 秒），并在交付时明确说明所做的假设，用户可要求调整重做。
- 有把握就做，没把握就按合理默认推进并在交付时说明假设；不每次都出 Plan，也不在关键输入缺失时强行执行。
